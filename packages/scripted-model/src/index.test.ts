...
describe('scripted model HTTP', () => {
  it('returns a navigate tool call for example.com', async () => {
    const app = createScriptedModelApp();
    const response = await app.request('/v1/chat/completions', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        messages: [{ role: 'user', content: 'navigate example.com' }],
      }),
    });
    const body = (await response.json()) as {
      choices: Array<{ message: { tool_calls?: Array<{ function: { name: string } }> } }>;
    };
    expect(body.choices[0]?.message.tool_calls?.[0]?.function.name).toBe('computer_navigate');
