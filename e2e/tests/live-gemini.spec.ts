...
const AUTH_EMULATOR = process.env.GABOT_AUTH_EMULATOR ?? 'http://127.0.0.1:9099';
const FIREBASE_API_KEY = process.env.GABOT_FIREBASE_API_KEY ?? 'demo-gabot';
...
const API_ONLY = process.env.GABOT_LIVE_API_ONLY === '1';
const PROMPT = `@monitor start long collaboration for ${String(MIN_TURNS + 2)} rounds investigating production anomalies. Keep auto-collaborating via delegate_to_bot across monitor, triage, and coder until the round budget completes.`;
...
test.describe('@live-gemini', () => {
  ...
  test('bot team auto-collaborates for 20+ turns on Vertex Gemini', async ({ page, request }) => {
    ...
    if (API_ONLY) {
      await triggerTurnViaApi(request);
    } else {
...
async function triggerTurnViaApi(request: APIRequestContext): Promise<void> {
  const token = await emulatorIdToken();
  const headers = {
  ...
  const profile = (await me.json()) as { defaultChannelId: string | null };
  const channelId = CHANNEL_ID || profile.defaultChannelId;
  expect(channelId).toBeTruthy();
  const turn = await request.post(`${API}/api/channels/${channelId}/turns`, {
    headers,
    data: { message: PROMPT },
  });
  expect(turn.ok()).toBeTruthy();
  // Drain SSE so the server keeps the turn alive; do not wait for completion here.
  await turn.text();
}
...
async function emulatorIdToken(): Promise<string> {
  const response = await fetch(
    `${AUTH_EMULATOR}/identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=${FIREBASE_API_KEY}`,
    {
      method: 'POST',
